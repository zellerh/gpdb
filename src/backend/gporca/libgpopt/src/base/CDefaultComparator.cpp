...
BOOL
CDefaultComparator::FUseInternalEvaluator(const IDatum *datum1, const IDatum *datum2,
										  BOOL *can_use_external_evaluator)
{
	*can_use_external_evaluator = GPOS_FTRACE(EopttraceEnableConstantExpressionEvaluation);
	...
	if (CUtils::FIntType(mdid1) &&
		CUtils::FIntType(mdid2) &&
	...
	{
		// Data types don't match exactly or no common mapping to LINT or double exists.
		// Be restrictive for now and only allow exact datatype matches on the two sides.
		result = false;
	}
	else
	{
	...
	if (false == result && *can_use_external_evaluator)
	{
		...
		*can_use_external_evaluator = CUtils::FConstrainableType(mdid1) && CUtils::FConstrainableType(mdid2);
	}
