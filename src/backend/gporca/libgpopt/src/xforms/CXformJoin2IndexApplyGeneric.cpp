...
CXformJoin2IndexApplyGeneric::Exfp
	(
...
// actual transform
void
CXformJoin2IndexApplyGeneric::Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) const
{
	...
	for (CExpression *pexprCurrInnerChild = pexprInner;
		 NULL == pexprGet;
		 pexprCurrInnerChild = ( NULL == pexprGet ? (*pexprCurrInnerChild)[0] : NULL))
	{
			...
			case COperator::EopLogicalProject:
				...
				// be used for index predicates.
				{
					...
					if (COperator::EopLogicalGbAgg == pexprCurrInnerChild->Pop()->Eopid())
					{
						...
						if (NULL != grbyAggOp->Pdrgpcr() && 0 < grbyAggOp->Pdrgpcr()->Size())
						{
	...
	if (pexprInner != pexprGet &&
		pexprInner != selectThatIsParentOfGet)
	{
